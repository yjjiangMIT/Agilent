...
namespace Agilent
{
    public partial class MainForm : Form
    {
        ...
        private static bool IS_WORK_MODE = true; // Set false if want to debug without serial connection.
        ...
        /** Stop unfinished job. */
        private void StopUnfinishedJob()
        {
            try
            {
                if (IS_WORK_MODE)
        ...
        private void ParseResolution()
        {
            ...
            else
            {
                ...
                foreach (string oneLine in this.richTextBoxMeasurement.Lines)
                {
                    if (oneLine.Length > 0)
                    {
                        emptyNumber++;
                ...
                foreach (string oneLine in this.richTextBoxResolution.Lines)
                {
                    if (oneLine.Length > 0)
                    {
                        emptyNumber--;
                ...
                emptyNumber -= this.outputCounter;
                Console.WriteLine(emptyNumber);
                Console.WriteLine(this.outputCounter);
